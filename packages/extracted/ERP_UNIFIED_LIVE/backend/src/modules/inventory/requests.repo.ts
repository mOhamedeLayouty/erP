...
export async function listIssueRequests(limit = 50, offset = 0) {
  ...
  try {
    ...
    return await db.query<any>(
      `SELECT TOP ${lim} START AT ${startAt} * FROM ${T.issue_h} ORDER BY debit_header DESC`
    );
  } finally { await db.close(); }
}

export async function listReturnRequests(limit = 50, offset = 0) {
  const db = await connectDb();
  try {
    const lim = Number(limit);
    const off = Math.max(0, Number(offset));
    const startAt = off + 1;
    return await db.query<any>(
      `SELECT TOP ${lim} START AT ${startAt} * FROM ${T.ret_h} ORDER BY credit_header DESC`
    );
  } finally { await db.close(); }
}
...
export async function listIssueRequestsFiltered(filters: FilterArgs) {
  const db = await connectDb();
  try {
    const lim = Number(filters.limit ?? 50);
    const off = Math.max(0, Number(filters.offset ?? 0));
    const startAt = off + 1; // SQL Anywhere START AT is 1-based
    const { clause, params } = buildWhere(filters, 'debit_date');
    return await db.query<any>(
      `SELECT TOP ${lim} START AT ${startAt} * FROM ${T.issue_h} ${clause} ORDER BY debit_header DESC`,
      params
    );
  } finally { await db.close(); }
}
...
export async function listReturnRequestsFiltered(filters: FilterArgs) {
  ...
  try {
    ...
    return await db.query<any>(
      `SELECT TOP ${lim} START AT ${startAt} * FROM ${T.ret_h} ${clause} ORDER BY credit_header DESC`,
      params
    );
  } finally { await db.close(); }
}
...
export type PostCheck = {
  ...
  reason?: string;
  stats?: LineStats;
  header_status?: RequestStatus;
  post_flag?: 'y'|'n'|null;
};

export async function getIssueHeader(debit_header: number) {
  const db = await connectDb();
  try {
    const rows = await db.query<any>(`SELECT debit_header, status, post_flag FROM ${T.issue_h} WHERE debit_header=?`, [debit_header]);
    return rows?.[0] || null;
  } finally { await db.close(); }
}

export async function getReturnHeader(credit_header: number) {
  const db = await connectDb();
  try {
    const rows = await db.query<any>(`SELECT credit_header, status, post_flag FROM ${T.ret_h} WHERE credit_header=?`, [credit_header]);
    return rows?.[0] || null;
  } finally { await db.close(); }
}

export async function canPostIssue(debit_header: number): Promise<PostCheck> {
  const hdr = await getIssueHeader(debit_header);
  if (!hdr) return { ok: false, reason: 'HEADER_NOT_FOUND' };
  if (String(hdr.post_flag || 'n').toLowerCase() === 'y') return { ok: false, reason: 'ALREADY_POSTED', header_status: hdr.status, post_flag: hdr.post_flag };
  const stats = await getIssueLineStats(debit_header);
  if (stats.pending > 0) return { ok: false, reason: 'PENDING_LINES', stats, header_status: hdr.status, post_flag: hdr.post_flag };
  if (Number(hdr.status) !== 1) return { ok: false, reason: 'HEADER_NOT_APPROVED', stats, header_status: hdr.status, post_flag: hdr.post_flag };
  if (stats.approved <= 0) return { ok: false, reason: 'NO_APPROVED_LINES', stats, header_status: hdr.status, post_flag: hdr.post_flag };
  return { ok: true, stats, header_status: hdr.status, post_flag: hdr.post_flag };
}

export async function canPostReturn(credit_header: number): Promise<PostCheck> {
...
async function countRows(table: string, clause: string, params: any[]) {
  const db = await connectDb();
  try {
    const rows = await db.query<any>(`SELECT COUNT(*) AS total FROM ${table} ${clause}`, params);
    return Number(rows?.[0]?.total ?? 0);
  } finally { await db.close(); }
}

export async function countIssueRequestsFiltered(filters: FilterArgs) {
  const { clause, params } = buildWhere(filters, 'debit_date');
  return await countRows(T.issue_h, clause, params);
}

export async function countReturnRequestsFiltered(filters: FilterArgs) {
  const { clause, params } = buildWhere(filters, 'credit_date');
  return await countRows(T.ret_h, clause, params);
}
