...
export async function listIssueRequests(limit = 50, offset = 0) {
    ...
    try {
        ...
        const startAt = off + 1;
        return await db.query(`SELECT TOP ${lim} START AT ${startAt} * FROM ${T.issue_h} ORDER BY debit_header DESC`);
    }
    finally {
        await db.close();
    }
...
export async function listReturnRequests(limit = 50, offset = 0) {
    ...
    try {
        ...
        const off = Math.max(0, Number(offset));
        const startAt = off + 1;
        return await db.query(`SELECT TOP ${lim} START AT ${startAt} * FROM ${T.ret_h} ORDER BY credit_header DESC`);
    }
    finally {
        await db.close();
    }
...
export async function listIssueRequestsFiltered(filters) {
    ...
    try {
        ...
        return await db.query(`SELECT TOP ${lim} START AT ${startAt} * FROM ${T.issue_h} ${clause} ORDER BY debit_header DESC`, params);
    }
    finally {
        await db.close();
    }
...
export async function listReturnRequestsFiltered(filters) {
    ...
    try {
        ...
        return await db.query(`SELECT TOP ${lim} START AT ${startAt} * FROM ${T.ret_h} ${clause} ORDER BY credit_header DESC`, params);
    }
    finally {
        await db.close();
    }
...
async function countRows(table, clause, params) {
    const db = await connectDb();
    try {
        const rows = await db.query(`SELECT COUNT(*) AS total FROM ${table} ${clause}`, params);
...
export async function countIssueRequestsFiltered(filters) {
    const { clause, params } = buildWhere(filters, 'debit_date');
    return await countRows(T.issue_h, clause, params);
}
export async function countReturnRequestsFiltered(filters) {
    const { clause, params } = buildWhere(filters, 'credit_date');
