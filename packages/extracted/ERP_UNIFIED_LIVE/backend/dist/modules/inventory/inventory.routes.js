...
import { listStores, listItems, listTransfers, listTransferDetails, createTransferHeader, createTransferDetail } from './inventory.repo.js';
import { postTransfer } from './stock-posting.service.js';
import { createIssueRequest, createReturnRequest } from './issue-return.repo.js';
import { listIssueRequests, listReturnRequests, getIssueDetails, getReturnDetails, setIssueStatus, setReturnStatus, setIssueLineStatus, setReturnLineStatus, listIssueRequestsFiltered, listReturnRequestsFiltered, approveAllIssueLines, approveAllReturnLines, rejectIssueLines, rejectReturnLines, getIssueLineStats, getReturnLineStats, syncIssueHeaderStatusFromLines, syncReturnHeaderStatusFromLines, canPostIssue, canPostReturn } from './requests.repo.js';
...
export function inventoryRouter(guardFactory) {
    const r = Router();
    const guard = guardFactory;
    // Reads
    r.get('/stores', guard('inventory.read'), async (_req, res, next) => {
        try {
            return res.json({ ok: true, data: await listStores(200) });
        }
        catch (e) {
            return next(e);
        }
    ...
    r.get('/items', guard('inventory.read'), async (_req, res, next) => {
        try {
            return res.json({ ok: true, data: await listItems(200) });
        }
        catch (e) {
            return next(e);
        }
    ...
    r.get('/transfers', guard('inventory.read'), async (_req, res, next) => {
        try {
            return res.json({ ok: true, data: await listTransfers(200) });
        }
        catch (e) {
            return next(e);
        }
    ...
    r.get('/transfer-details', guard('inventory.read'), async (_req, res, next) => {
        try {
            return res.json({ ok: true, data: await listTransferDetails(500) });
        }
        catch (e) {
            return next(e);
        }
    ...
    r.post('/transfers', guard('inventory.write'), validateBody(anyObject), async (req, res, next) => {
        try {
            const affected = await createTransferHeader(req.body);
    ...
    r.post('/transfer-details', guard('inventory.write'), validateBody(anyObject), async (req, res, next) => {
        try {
            const affected = await createTransferDetail(req.body);
            return res.json({ ok: true, data: { affected } });
        }
        catch (e) {
            return next(e);
        ...
        try {
            ...
            if (hasAny) {
                const data = await listIssueRequestsFiltered({
                    ...
                    status: (typeof q.status !== 'undefined' && q.status !== '') ? Number(q.status) : undefined,
                    post_flag: q.post_flag ? String(q.post_flag) : undefined,
    ...
    r.post('/issue-requests/:debit_header/action', guard('inventory.write'), validateBody(actionSchema), async (req, res, next) => {
        try {
            ...
            if (action === 'approve')
                return res.json({ ok: true, data: await setIssueStatus(debit_header, 1) });
            if (action === 'reject')
                return res.json({ ok: true, data: await setIssueStatus(debit_header, 2) });
            if (action === 'post') {
                const actor = (req.user?.user_name ?? req.user?.user_id ?? 'SYSTEM');
                const data = await postIssueRequest(debit_header, actor);
                return res.json({ ok: true, data });
            }
            if (action === 'unpost')
                return res.json({ ok: true, data: await setIssueStatus(debit_header, 1, 'n') });
    ...
    r.post('/issue-requests/:debit_header/lines/:debit_detail/action', guard('inventory.write'), validateBody(lineActionSchema), async (req, res, next) => {
        try {
            ...
            if (action === 'approve') {
                try {
                    await insertAudit({ entity: 'ISSUE', header_id: debit_header, line_id: debit_detail, action: `LINE_${String(action).toUpperCase()}`, reason: reason || null, note: note || null, actor: req.user?.username || null });
                }
                catch { /* ignore audit */ }
    ...
    r.post('/return-requests/:credit_header/action', guard('inventory.write'), validateBody(actionSchema), async (req, res, next) => {
        try {
            ...
            const { action } = req.body;
            if (action === 'approve')
                return res.json({ ok: true, data: await setReturnStatus(credit_header, 1) });
            if (action === 'reject')
                return res.json({ ok: true, data: await setReturnStatus(credit_header, 2) });
            if (action === 'post') {
                const actor = (req.user?.user_name ?? req.user?.user_id ?? 'SYSTEM');
            ...
            if (action === 'unpost')
                return res.json({ ok: true, data: await setReturnStatus(credit_header, 1, 'n') });
            return res.json({ ok: false });
        }
        catch (e) {
            return next(e);
    ...
    r.post('/return-requests/:credit_header/lines/:credit_detail/action', guard('inventory.write'), validateBody(lineActionSchema), async (req, res, next) => {
        try {
            ...
            if (action === 'approve') {
                try {
                    await insertAudit({ entity: 'RETURN', header_id: credit_header, line_id: credit_detail, action: `LINE_${String(action).toUpperCase()}`, reason: reason || null, note: note || null, actor: req.user?.username || null });
                }
                catch { /* ignore audit */ }
    ...
    r.get('/lost-sales', guard('inventory.read'), async (_req, res, next) => {
        try {
            return res.json({ ok: true, data: await listLostSales(200) });
        }
        catch (e) {
            return next(e);
        }
    ...
    r.get('/balances', guard('inventory.read'), async (_req, res, next) => {
        try {
            return res.json({ ok: true, data: await listBalances(200) });
        }
        catch (e) {
            return next(e);
        }
    ...
    r.post('/issue-requests/:debit_header/lines/approve-all', guard('inventory.write'), async (req, res, next) => {
        try {
            ...
            try {
                await insertAudit({ entity: 'ISSUE', header_id: debit_header, line_id: null, action: 'BULK_APPROVE_ALL', actor: req.user?.username || null });
            }
            catch { /* ignore audit */ }
    ...
    r.post('/issue-requests/:debit_header/lines/reject', guard('inventory.write'), validateBody(bulkRejectSchema), async (req, res, next) => {
        try {
            const debit_header = Number(req.params.debit_header);
            const { line_ids, reason, note } = req.body;
            await syncIssueHeaderStatusFromLines(debit_header);
            try {
                await insertAudit({ entity: 'ISSUE', header_id: debit_header, line_id: null, action: 'BULK_REJECT', reason: reason || null, note: note || null, actor: req.user?.username || null });
            }
            catch { /* ignore audit */ }
    ...
    r.post('/return-requests/:credit_header/lines/approve-all', guard('inventory.write'), async (req, res, next) => {
        try {
            ...
            try {
                await insertAudit({ entity: 'RETURN', header_id: credit_header, line_id: null, action: 'BULK_APPROVE_ALL', actor: req.user?.username || null });
            }
            catch { /* ignore audit */ }
    ...
    r.post('/return-requests/:credit_header/lines/reject', guard('inventory.write'), validateBody(bulkRejectSchema), async (req, res, next) => {
        try {
            ...
            await syncReturnHeaderStatusFromLines(credit_header);
            try {
                await insertAudit({ entity: 'RETURN', header_id: credit_header, line_id: null, action: 'BULK_REJECT', reason: reason || null, note: note || null, actor: req.user?.username || null });
            }
            catch { /* ignore audit */ }
    ...
    r.get('/issue-requests/:debit_header/export.csv', guard('inventory.read'), async (req, res, next) => {
        try {
            ...
            const rows = details.map((d) => ({ debit_header, ...d }));
    ...
    r.get('/return-requests/:credit_header/export.csv', guard('inventory.read'), async (req, res, next) => {
        try {
            ...
            const rows = details.map((d) => ({ credit_header, ...d }));
            const csv = toCsv(rows);
            res.setHeader('Content-Type', 'text/csv; charset=utf-8');
    ...
    function htmlTable(title, id, rows) {
  ...
  <tbody>${rows.map((r) => `<tr>${cols.map(k => `<td>${esc(r[k])}</td>`).join('')}</tr>`).join('')}</tbody></table>
  <script>window.print()</script>
    ...
    r.post('/issue-requests/:debit_header/sync', guard('inventory.write'), async (req, res, next) => {
        try {
            const debit_header = Number(req.params.debit_header);
            try {
                await insertAudit({ entity: 'ISSUE', header_id: debit_header, line_id: null, action: 'HEADER_SYNC', actor: req.user?.username || null });
            }
            catch { /* ignore audit */ }
    ...
    r.post('/return-requests/:credit_header/sync', guard('inventory.write'), async (req, res, next) => {
        try {
            const credit_header = Number(req.params.credit_header);
            try {
                await insertAudit({ entity: 'RETURN', header_id: credit_header, line_id: null, action: 'HEADER_SYNC', actor: req.user?.username || null });
            }
            catch { /* ignore audit */ }
