...
export async function getJob(jobOrderId) {
    const db = await connectDb();
    try {
        const rows = await db.query(`SELECT TOP 1 * FROM ${T_JOB} WHERE JobOrderID = ?`, [jobOrderId]);
        return rows[0] ?? null;
    }
    finally {
        await db.close();
...
export async function applyTransition(jobOrderId, action) {
    if (!jobOrderId)
        throw badRequest('JobOrderID is required');
    const tr = TRANSITIONS[action];
    if (!tr)
        throw badRequest('Unknown action');
    const current = await getJob(jobOrderId);
    if (!current)
        throw badRequest('Job order not found');
    ...
    const db = await connectDb();
    try {
        const affected = await db.exec(`UPDATE ${T_JOB} SET ${sets} WHERE JobOrderID = ?`, [tr.to, jobOrderId]);
        return affected;
    }
    finally {
        await db.close();
