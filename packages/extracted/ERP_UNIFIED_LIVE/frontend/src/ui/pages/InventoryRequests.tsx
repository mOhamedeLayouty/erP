...
export default function InventoryRequests() {
  ...
  function buildQuery() {
    ...
    // pagination always
    q.limit = String(pageLimit);
    q.offset = String(pageOffset);
  ...
  async function load() {
    ...
    try {
      ...
      if (payload?.rows) {
        ...
        setPageTotal(Number(payload.total ?? 0));
      } else if (payload?.data?.rows) {
        setRows(payload.data.rows);
        setPageTotal(Number(payload.data.total ?? 0));
      } else {
        setRows(payload);
        setPageTotal(Array.isArray(payload) ? payload.length : 0);
      }
  ...
  useEffect(() => { load(); }, [tab, pageLimit, pageOffset, api.baseUrl, api.token]);
  ...
  return (
    <div>
      ...
      <Callout title="Phase 3.16 (Pagination)">
        Pagination للـ Requests List (limit/offset) + Prev/Next.
      </Callout>
      ...
      <div style={{ border: '1px solid #eee', padding: 12, borderRadius: 12, marginBottom: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 10, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <Button onClick={() => { setPageOffset(0); load(); }} disabled={busy || loading}>Apply</Button>
          <Button onClick={() => { setFStore(''); setFJob(''); setFStatus(''); setFPost(''); setFFrom(''); setFTo(''); setPageOffset(0); }} disabled={busy || loading}>Clear</Button>

          <div style={{ marginLeft: 'auto', display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
            <span style={{ color: '#666', fontSize: 12 }}>Rows:</span>
            <Select value={String(pageLimit)} onChange={(e) => { setPageLimit(Number(e.target.value)); setPageOffset(0); }}>
              <option value="25">25</option>
              <option value="50">50</option>
              <option value="100">100</option>
              <option value="200">200</option>
            </Select>
            <span style={{ color: '#666', fontSize: 12 }}>Offset: {pageOffset} / Total: {pageTotal}</span>
            <Button onClick={() => setPageOffset(Math.max(0, pageOffset - pageLimit))} disabled={busy || loading || pageOffset<=0}>Prev</Button>
            <Button onClick={() => { if (pageOffset + pageLimit < pageTotal) setPageOffset(pageOffset + pageLimit); }} disabled={busy || loading || (pageOffset + pageLimit >= pageTotal)}>Next</Button>
          </div>
        </div>
      </div>
