...
import { postIssueRequest, postReturnRequest, unpostIssueRequest, unpostReturnRequest } from './request-posting.service.js';
...
export function inventoryRouter(guardFactory: PermissionGuardFactory) {
  ...
  r.post('/issue-requests/:debit_header/action', guard('inventory.write'), validateBody(actionSchema), async (req, res, next) => {
    try {
      ...
      if (action === 'unpost') {
        const actor = (req.user?.user_name ?? req.user?.user_id ?? 'SYSTEM') as string;
        const data = await unpostIssueRequest(debit_header, actor);
        try { await insertAudit({ entity:'ISSUE', header_id: debit_header, line_id: null, action: 'HEADER_UNPOST', actor }); } catch { /* ignore audit */ }
        return res.json({ ok: true, data });
      }
  ...
  r.post('/return-requests/:credit_header/action', guard('inventory.write'), validateBody(actionSchema), async (req, res, next) => {
    try {
      ...
      if (action === 'unpost') {
        const actor = (req.user?.user_name ?? req.user?.user_id ?? 'SYSTEM') as string;
        const data = await unpostReturnRequest(credit_header, actor);
        try { await insertAudit({ entity:'RETURN', header_id: credit_header, line_id: null, action: 'HEADER_UNPOST', actor }); } catch { /* ignore audit */ }
        return res.json({ ok: true, data });
      }
