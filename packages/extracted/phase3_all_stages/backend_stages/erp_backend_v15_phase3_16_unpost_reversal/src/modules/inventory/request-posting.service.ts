...
function extractPostedId(notes: any, key: string): number | null {
...
async function adjustBalance(db: any, store_id: number, item_id: string, delta: number, sc: number, loc: number) {
  // Best-effort: update existing row, else insert.
  await db.exec(
    `UPDATE ${T.balance}
     SET balance = COALESCE(balance,0) + ?
     WHERE store_id=? AND item_id=? AND service_center=? AND location_id=?`,
    [delta, store_id, item_id, sc, loc]
  );
  const rows = await db.query<any>(
    `SELECT COUNT(*) AS c FROM ${T.balance} WHERE store_id=? AND item_id=? AND service_center=? AND location_id=?`,
    [store_id, item_id, sc, loc]
  );
  const c = Number(rows?.[0]?.c ?? 0);
  if (c <= 0) {
    await db.exec(
      `INSERT INTO ${T.balance}(store_id, item_id, balance, service_center, location_id)
       VALUES(?,?,?,?,?)`,
      [store_id, item_id, delta, sc, loc]
    );
  }
}

async function deleteLostSalesByRequest(db: any, request_no: string, sc: number, loc: number) {
  // Avoid duplicates when reposting after unpost
  await db.exec(
    `DELETE FROM ${T.lost_sales} WHERE request_no=? AND service_center=? AND location_id=?`,
    [request_no, sc, loc]
  );
}

export async function unpostIssueRequest(debit_header_request: number, actorUser: string) {
  if (!debit_header_request) throw badRequest('debit_header_request required');
  ...
  try {
    ...
    if (String(hdr.post_flag ?? '').toLowerCase() !== 'y') {
      return { already_unposted: true, debit_header: null, reversed_lines: 0 };
    }

    const postedId = extractPostedId(hdr.notes, 'POSTED_TO:sc_debit_header');
    if (!postedId) throw badRequest('Cannot unpost: posted document id not found in notes');

    // Load posted lines
    const det = await db.query<any>(
      `SELECT * FROM ${T.issue_d} WHERE debit_header=? AND service_center=? AND location_id=?`,
      [postedId, sc, loc]
    );

    // Reverse balance (Issue: posting decreases, unpost increases back)
    let reversed_lines = 0;
    for (const line of det) {
      const item_id = String(line.item_id ?? '');
      const qty = Number(line.qty ?? 0);
      if (!item_id || !qty) continue;
      await adjustBalance(db, Number(hdr.store_id), item_id, qty, sc, loc);
      reversed_lines += 1;
    }

    // Delete posted docs (best-effort)
    await db.exec(`DELETE FROM ${T.issue_d} WHERE debit_header=? AND service_center=? AND location_id=?`, [postedId, sc, loc]);
    await db.exec(`DELETE FROM ${T.issue_h} WHERE debit_header=? AND service_center=? AND location_id=?`, [postedId, sc, loc]);

    // Remove lost sales rows for this request_no (so repost doesn't duplicate)
    await deleteLostSalesByRequest(db, String(debit_header_request).slice(0, 10), sc, loc);

    // Update request header back to unposted
    await db.exec(
      `UPDATE ${T.issue_h_req}
       SET post_flag='n',
           notes = CASE
       ...
       WHERE debit_header=? AND service_center=? AND location_id=?`,
      [
        `UNPOSTED_FROM:sc_debit_header=${postedId} BY:${actorUser}`,
        `UNPOSTED_FROM:sc_debit_header=${postedId} BY:${actorUser}`,
        debit_header_request, sc, loc
      ]
    );

    return { ok: true, debit_header: postedId, reversed_lines };
  } finally {
    await db.close();
  }
}

export async function unpostReturnRequest(credit_header_request: number, actorUser: string) {
  if (!credit_header_request) throw badRequest('credit_header_request required');
  ...
  try {
    ...
    if (String(hdr.post_flag ?? '').toLowerCase() !== 'y') {
      return { already_unposted: true, credit_header: null, reversed_lines: 0 };
    }

    const postedId = extractPostedId(hdr.notes, 'POSTED_TO:sc_credit_header');
    if (!postedId) throw badRequest('Cannot unpost: posted document id not found in notes');

    const det = await db.query<any>(
      `SELECT * FROM ${T.ret_d} WHERE credit_header=? AND service_center=? AND location_id=?`,
      [postedId, sc, loc]
    );

    // Reverse balance (Return: posting increases, unpost decreases)
    let reversed_lines = 0;
    for (const line of det) {
      const item_id = String(line.item_id ?? '');
      const qty = Number(line.qty ?? 0);
    ...
    await db.exec(`DELETE FROM ${T.ret_d} WHERE credit_header=? AND service_center=? AND location_id=?`, [postedId, sc, loc]);
    await db.exec(`DELETE FROM ${T.ret_h} WHERE credit_header=? AND service_center=? AND location_id=?`, [postedId, sc, loc]);

    // Update request header back to unposted (keep return request history)
    await db.exec(
      `UPDATE ${T.ret_h_req}
       SET post_flag='n',
           notes = CASE
       ...
       WHERE credit_header=? AND service_center=? AND location_id=?`,
      [
        `UNPOSTED_FROM:sc_credit_header=${postedId} BY:${actorUser}`,
        `UNPOSTED_FROM:sc_credit_header=${postedId} BY:${actorUser}`,
        credit_header_request, sc, loc
      ]
    );

    return { ok: true, credit_header: postedId, reversed_lines };
  } finally {
    await db.close();
  }
}
