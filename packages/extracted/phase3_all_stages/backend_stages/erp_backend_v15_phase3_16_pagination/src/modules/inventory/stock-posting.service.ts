...
export async function postTransfer(transferId: string) {
  if (!transferId) throw badRequest('transferId is required');

  const db = await connectDb();
  try {
    const hdr = await db.query<any>(`SELECT TOP 1 * FROM ${T_HDR} WHERE 1=1 AND ${infer_id_col()} = ?`, [transferId]);
    if (!hdr[0]) throw badRequest('transfer header not found');

    const dtl = await db.query<any>(`SELECT TOP 1 * FROM ${T_DTL} WHERE 1=1 AND ${infer_fk_col()} = ?`, [transferId]);
    if (!dtl[0]) throw badRequest('transfer has no details');
