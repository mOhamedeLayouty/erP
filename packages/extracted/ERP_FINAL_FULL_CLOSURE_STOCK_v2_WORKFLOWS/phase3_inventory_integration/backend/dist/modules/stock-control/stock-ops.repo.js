...
async function begin(db) { await db.exec('BEGIN TRANSACTION'); }
async function commit(db) { await db.exec('COMMIT'); }
async function rollback(db) { try {
    await db.exec('ROLLBACK');
}
catch { /* ignore */ } }
async function lastIdentity(db) {
    // SQL Anywhere: @@identity
    const r = await db.query('SELECT @@identity AS id');
    const id = Number(r?.[0]?.id ?? 0);
    if (!id)
        throw new AppError('DB_IDENTITY', 500, 'Failed to read @@identity after insert');
    return id;
}
export async function createPurchaseOrder(input, lines) {
    if (!input?.vend_code)
        throw new AppError('VALIDATION', 400, 'vend_code is required');
    if (!input?.store_id)
        throw new AppError('VALIDATION', 400, 'store_id is required');
    if (!Array.isArray(lines) || lines.length === 0)
        throw new AppError('VALIDATION', 400, 'lines is required');
    ...
    try {
        ...
        await db.exec(`INSERT INTO DBA.sc_buy_order_header
       ...
       VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`, [
        ...
        for (const ln of lines) {
            if (!ln.item_id)
                throw new AppError('VALIDATION', 400, 'line.item_id is required');
            if (ln.qty === undefined || ln.qty === null)
                throw new AppError('VALIDATION', 400, 'line.qty is required');
            await db.exec(`INSERT INTO DBA.sc_buy_order_detail
...
export async function setPurchaseOrderApproved(buy_header, approved, approved_by) {
    const db = await connectDb();
    try {
        const entryDate = nowDateTime();
        await db.exec(`UPDATE DBA.sc_buy_order_header
       SET approved=?, approved_by=?, edit_user=?, edit_date=?
       WHERE buy_header=?`, [approved ? 1 : 0, approved_by ?? null, approved_by ?? null, entryDate, buy_header]);
        return { ok: true };
    }
    finally {
        await db.close();
...
export async function createReceipt(input, lines) {
    if (!input?.store_id)
        throw new AppError('VALIDATION', 400, 'store_id is required');
    if (!Array.isArray(lines) || lines.length === 0)
        throw new AppError('VALIDATION', 400, 'lines is required');
    ...
    try {
        ...
        await db.exec(`INSERT INTO DBA.sc_credit_header
       ...
       VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`, [
        ...
        for (const ln of lines) {
            ...
            if (ln.qty === undefined || ln.qty === null)
                throw new AppError('VALIDATION', 400, 'line.qty is required');
            await db.exec(`INSERT INTO DBA.sc_credit_detail
...
export async function postReceipt(credit_header, user_name) {
    const db = await connectDb();
    try {
        const t = nowDateTime();
        await db.exec(`UPDATE DBA.sc_credit_header
       SET post_flag=1, user_name=COALESCE(?, user_name), trans_time=?
       WHERE credit_header=?`, [user_name ?? null, t, credit_header]);
        return { ok: true };
    }
    finally {
        await db.close();
...
export async function createIssue(input, lines) {
    if (!input?.store_id)
        throw new AppError('VALIDATION', 400, 'store_id is required');
    if (!Array.isArray(lines) || lines.length === 0)
        throw new AppError('VALIDATION', 400, 'lines is required');
    ...
    try {
        ...
        const transTime = nowDateTime();
        await db.exec(`INSERT INTO DBA.sc_debit_header
       ...
       VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`, [
        ...
        for (const ln of lines) {
            await db.exec(`INSERT INTO DBA.sc_debit_detail
...
export async function postIssue(debit_header, user_name) {
    const db = await connectDb();
    try {
        const t = nowDateTime();
        await db.exec(`UPDATE DBA.sc_debit_header
       SET post_flag=1, user_name=COALESCE(?, user_name), confirm_date=COALESCE(confirm_date, ?)
       WHERE debit_header=?`, [user_name ?? null, t, debit_header]);
        return { ok: true };
    }
    finally {
        await db.close();
...
export async function createTransfer(input, lines) {
    if (!input?.from_store_id)
        throw new AppError('VALIDATION', 400, 'from_store_id is required');
       ...
       VALUES (?,?,?,?,?,?,?,?)`, [
        ...
        for (const ln of lines) {
         ...
         VALUES (?,?,?,?,?,?,?,?,?,?)`, [
            ...
            await db.exec(`INSERT INTO DBA.sc_transfer_detail
...
export async function postTransfer(credit_header, user_name) {
    // Same as postReceipt for transfer credit doc
    return postReceipt(credit_header, user_name);
}
export async function getBalance(store_id, item_id, location_id, service_center, limit = 500) {
    const db = await connectDb();
    try {
        const where = [];
        const params = [];
        ...
        if (service_center !== undefined && service_center !== null && service_center !== '') {
            where.push('service_center=?');
            params.push(service_center);
        }
