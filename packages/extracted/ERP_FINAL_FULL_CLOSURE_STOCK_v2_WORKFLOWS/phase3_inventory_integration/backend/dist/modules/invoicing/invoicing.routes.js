...
export function invoicingRouter(guard) {
    const r = Router();
    r.get('/invoices', guard('invoice.read'), async (_req, res, next) => {
        try {
            return res.json({ ok: true, data: await listInvoices(200) });
        }
        catch (e) {
            return next(e);
        }
    ...
    r.get('/invoices/:InvoiceID/details', guard('invoice.read'), async (req, res, next) => {
        try {
            return res.json({ ok: true, data: await listInvoiceDetails(req.params.InvoiceID, 500) });
        }
        catch (e) {
            return next(e);
        }
    ...
    r.post('/invoices', guard('invoice.write'), validateBody(anyObject), async (req, res, next) => {
        try {
            const affected = await createInvoiceHeader(req.body);
    ...
    r.post('/invoice-details', guard('invoice.write'), validateBody(anyObject), async (req, res, next) => {
        try {
            const affected = await createInvoiceDetail(req.body);
            return res.json({ ok: true, data: { affected } });
        }
        catch (e) {
            return next(e);
