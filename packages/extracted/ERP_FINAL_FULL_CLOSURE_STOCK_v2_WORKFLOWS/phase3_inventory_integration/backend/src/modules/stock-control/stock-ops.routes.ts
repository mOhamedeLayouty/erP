 ...
 */
export function stockOpsRouter(_guard: PermissionGuardFactory) {
  const r = Router();
  ...
  r.put('/po/:id/approve', async (req, res, next) => {
    try {
      ...
      if (!id) throw badRequest('Invalid PO id');
  ...
  r.put('/receipt/:id/post', async (req, res, next) => {
    try {
      const id = Number(req.params.id);
      if (!id) throw badRequest('Invalid receipt id');
  ...
  r.put('/issue/:id/post', async (req, res, next) => {
    try {
      const id = Number(req.params.id);
      if (!id) throw badRequest('Invalid issue id');
      const { user_name } = req.body ?? {};
  ...
  r.put('/transfer/:id/post', async (req, res, next) => {
    try {
      const id = Number(req.params.id);
      if (!id) throw badRequest('Invalid transfer id');
      const { user_name } = req.body ?? {};
  ...
  r.get('/balance', async (req, res, next) => {
    try {
      const { store_id, item_id, location_id, service_center, limit } = req.query as any;
      const result = await getBalance(
        store_id ? String(store_id) : undefined,
        item_id ? String(item_id) : undefined,
