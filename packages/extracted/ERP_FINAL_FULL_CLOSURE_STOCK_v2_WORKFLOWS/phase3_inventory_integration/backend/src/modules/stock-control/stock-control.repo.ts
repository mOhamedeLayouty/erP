...
function assertLimit(n: unknown, def = 200): number {
  const v = Number(n ?? def);
  if (!Number.isFinite(v) || v <= 0) return def;
  return Math.min(Math.floor(v), 1000);
}

export async function listRows(tableKey: StockControlTableKey, limit: unknown) {
...
export async function getTableColumns(tableKey: StockControlTableKey) {
  ...
  try {
    ...
    const rows2 = await db.query<{ column_name: string }>(
    ...
    if (rows2?.length) return rows2.map((r) => r.column_name);
...
function sanitizeKeys(obj: any, allowedColumns: string[]) {
...
export async function insertRow(tableKey: StockControlTableKey, payload: any) {
  const cols = await getTableColumns(tableKey);
  const clean = sanitizeKeys(payload, cols);
  const keys = Object.keys(clean);
  if (!keys.length) throw badRequest('No valid columns provided for insert');

  const table = qualify(STOCK_CONTROL_TABLES[tableKey]);
  const placeholders = keys.map(() => '?').join(',');
  const sql = `INSERT INTO ${table} (${keys.join(',')}) VALUES (${placeholders})`;
  const params = keys.map((k) => (clean as any)[k]);
...
export async function updateRow(
  tableKey: StockControlTableKey,
  pkColumn: string,
  pkValue: any,
  payload: any
) {
  if (!pkColumn) throw badRequest('pkColumn is required');
  const cols = await getTableColumns(tableKey);
  const clean = sanitizeKeys(payload, cols);
  const keys = Object.keys(clean).filter((k) => k.toLowerCase() !== pkColumn.toLowerCase());
  if (!keys.length) throw badRequest('No valid columns provided for update');

  const table = qualify(STOCK_CONTROL_TABLES[tableKey]);
  const sets = keys.map((k) => `${k} = ?`).join(', ');
  const sql = `UPDATE ${table} SET ${sets} WHERE ${pkColumn} = ?`;
  const params = [...keys.map((k) => (clean as any)[k]), pkValue];
...
export async function deleteRow(tableKey: StockControlTableKey, pkColumn: string, pkValue: any) {
  if (!pkColumn) throw badRequest('pkColumn is required');
  const table = qualify(STOCK_CONTROL_TABLES[tableKey]);
  const db = await connectDb();
  try {
