...
export async function listCustomers(limit = 200) {
    ...
    try {
        return await db.query(`SELECT TOP ${limit} customer_id, customer_name_a, customer_name_e, GSM, email, address FROM ${T_CUSTOMER} ORDER BY customer_id`);
    }
    finally {
        await db.close();
    }
}
export async function createCustomer(input, actor_user_id) {
    ...
    for (const k of needed) {
        if (!cols.includes(k))
            throw new Error('Locked schema mismatch: missing ' + k);
    }
...
export async function listEquipment(limit = 200) {
    const db = await connectDb();
    try {
        return await db.query(`SELECT TOP ${limit} eqpt_id, licence_no, vin_no, year, eqpt_number, service_center FROM ${T_EQPT} ORDER BY eqpt_id`);
    }
    finally {
        await db.close();
    }
