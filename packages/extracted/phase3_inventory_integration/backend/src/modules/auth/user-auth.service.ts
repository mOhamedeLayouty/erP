...
type UserRolesConfig = {
  by_user_id?: Record<string, string[]>;
  by_user_name?: Record<string, string[]>;
};

type UserSecretsConfig = {
  strategy?: 'plain' | 'sha256';
  by_user_id?: Record<string, string>;
  by_user_name?: Record<string, string>;
};
...
function loadUserRoles(): UserRolesConfig {
  return loadJsonIfExists<UserRolesConfig>(process.env.USER_ROLES_JSON_PATH ?? './config/user_roles.json') ?? {};
}

function loadUserSecrets(): UserSecretsConfig {
  return loadJsonIfExists<UserSecretsConfig>(process.env.USER_SECRETS_JSON_PATH ?? './config/user_secrets.json') ?? {};
}

function resolveRoles(user: Record<string, any>): string[] {
  ...
  if (rolesById?.length) return rolesById;

  const rolesByName = (uname && cfg.by_user_name?.[String(uname).toLowerCase()]) ?? null;
  if (rolesByName?.length) return rolesByName;
...
export function signAccessToken(payload: any) {
  ...
  return jwt.sign(payload, (process.env.JWT_SECRET ?? 'change_me') as any, opts);
}
...
export function signRefreshToken(payload: any) {
  ...
  return jwt.sign({ ...payload, token_type: 'refresh' }, (process.env.JWT_SECRET ?? 'change_me') as any, opts);
}


function verifyPassword(stored: string, password: string, strategy: string): boolean {
  const s = (strategy ?? 'plain').toLowerCase();
  if (s === 'plain') return stored === password;
  if (s === 'sha256') return stored.toLowerCase() === sha256Hex(password).toLowerCase();
  throw new Error('Unsupported PASSWORD_STRATEGY: ' + s);
}

export async function login(user_name: string, password: string) {
  if (!user_name) throw badRequest('user_name is required');
  ...
  if (passCol) {
    ...
    if (stored == null) throw unauthorized('Invalid credentials');
    const strategy = process.env.PASSWORD_STRATEGY ?? 'plain';
    if (!verifyPassword(String(stored), password, strategy)) throw unauthorized('Invalid credentials');
  } else {
    ...
    if (!stored) throw unauthorized('Auth not configured (no password column; missing user_secrets)');
    if (!verifyPassword(String(stored), password, strategy)) throw unauthorized('Invalid credentials');
  }
...
export function refresh(refresh_token: string) {
  ...
  if (decoded?.token_type !== 'refresh') throw unauthorized('Invalid refresh token');
