...
export async function listInvoiceDetails(invoiceId: string, limit = 500) {
  const db = await connectDb();
  try {
...
export async function createInvoiceHeader(payload: Record<string, any>) {
  const db = await connectDb();
  try {
    const t = INVOICING_TABLES.header.table;
...
export async function createInvoiceDetail(payload: Record<string, any>) {
  const db = await connectDb();
  try {
    const t = INVOICING_TABLES.detail.table;
