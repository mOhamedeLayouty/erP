-- TRIGGER: DBA.tr_doc_son_details_paidamount
-- ON TABLE: DBA.doc_son_rec_details
-- generated_at: 2025-12-29T13:52:33.693Z

create trigger tr_doc_son_details_paidamount after insert,delete,update order 2 on
DBA.doc_son_rec_details
referencing old as old_rec new as new_rec
for each row
//V1.0
//V1.1 handle value if record deleted
//V1.2 Link by invoiceid instead invoice No.
//V1.3 sub downpayment if sub form Job ,ignor delete receipt
//V1.4 add details sub
begin
  if(new_rec.InvoiceID is not null and new_rec.InvoiceID <> '') or(old_rec.InvoiceID is not null and old_rec.InvoiceID <> '') then
    update ws_receipt set ws_receipt.paidamount
       = isnull((select sum(paymentAmount) from ws_ReceiptDetail
      ...
      +(isnull((select Sum((case doc_type when 1 then doc_tot*-1 else doc_tot end)) from doc_son_rec
        where doc_son_rec.customer_id = ws_receipt.custid
      ...
      +isnull((select sum((case doc_type when 1 then paid_amount*-1 else paid_amount end))
        from doc_son_rec_details
        where doc_son_rec_details.customer_id = ws_receipt.custid
        ...
        and(doc_son_rec_details.InvoiceType = ws_receipt.InvoiceType)),0))
      ...
      /* and round(isnull(ws_receipt.paidamount,0),2) <> round(ws_receipt.receipt_amount,2)
and isnull(ws_receipt.paidamount,0)
+(isnull((select Sum(doc_tot) from doc_son_rec
where doc_son_rec.doc_type = 2 
and doc_son_rec.customer_id = ws_receipt.custid
and doc_son_rec.service_center = ws_receipt.service_center
and doc_son_rec.main_location_id = ws_receipt.location_id
and doc_son_rec.invoiceno = ws_receipt.invoiceno),0)
-isnull((select Sum(doc_tot) from doc_son_rec
where doc_son_rec.doc_type = 1 
and doc_son_rec.customer_id = ws_receipt.custid
and doc_son_rec.service_center = ws_receipt.service_center
...
and(doc_son_rec_details.service_center = ws_receipt.service_center)
and(doc_son_rec_details.main_location_id = ws_receipt.location_id)
and(doc_son_rec_details.invoiceno = ws_receipt.invoiceno)),0))
<> round(ws_receipt.paidamount,2)*/
      and isnull((select sum(paymentAmount) from ws_ReceiptDetail
        where ws_ReceiptDetail.receipt_id = ws_Receipt.receipt_id
        and ws_ReceiptDetail.service_center = ws_Receipt.service_center
        and ws_ReceiptDetail.main_location_id = ws_Receipt.location_id),0)
      +(isnull((select Sum((case doc_type when 1 then doc_tot*-1 else doc_tot end)) from doc_son_rec
    ...
    update ws_receipt set ws_receipt.ReceiptType = 'Y'
      ...
      and ws_receipt.custid = new_rec.customer_id
      and isnull(ws_receipt.delete_flag,'N') <> 'Y'
      and ws_receipt.ReceiptType = 'N' and round(ws_receipt.paidamount,2) < round(ws_receipt.receipt_amount,2)
  end if
end
