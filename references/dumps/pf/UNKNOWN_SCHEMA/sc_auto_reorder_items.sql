-- PF: UNKNOWN_SCHEMA.sc_auto_reorder_items
-- proc_id: 420
-- generated_at: 2025-12-29T13:53:28.813Z

create procedure DBA.sc_auto_reorder_items( @s_center integer )  /* [IN | OUT | INOUT] parameter_name parameter_type [DEFAULT default_value], ... */
...
begin
  ...
  select sc_auto_reorder.item_id,sc_auto_reorder.service_center,sc_auto_reorder.location_id,
    ...
    sc_item.qty_factor,sc_item.storage_cost,sc_auto_reorder.ord_factor,from_date,to_date,no_month,lead_time,safety_stock_time,Jan_sales=1.1111111111111,Feb_sales=1.1111111111111,
    Mar_sales=1.1111111111111,Apr_sales=1.1111111111111,May_sales=1.1111111111111,
    Jun_sales=1.1111111111111,Jul_sales=1.1111111111111,Aug_sales=1.1111111111111,Sep_sales=1.1111111111111,Oct_sales=1.1111111111111,
    ...
    into #t
    from sc_item left outer join monthly_sales_avg on sc_item.item_id = monthly_sales_avg.item_id and sc_item.service_center = monthly_sales_avg.service_center
      ,sc_auto_reorder where(sc_item.item_id = sc_auto_reorder.item_id) and(sc_item.service_center = sc_auto_reorder.service_center)
    and(sc_auto_reorder.service_center = @s_center) order by sc_item.sparepart asc
  set @rcount = @@rowcount
  ...
  while(@rcount > 0)
    begin
      ...
      select @m1 = isnull((select Sum(a.debit)/(case @all_out when 0 then 1 else @all_out end) from DBA.sc_month_balance as a where a.item_id = sc_month_balance.item_id and Year(a.b_date) = Year(Getdate()) and month(a.b_date) = 1 and sc_month_balance.service_center = a.service_center),0.0),
        @m2 = isnull((select Sum(a.debit)/(case @all_out when 0 then 1 else @all_out end) from DBA.sc_month_balance as a where a.item_id = sc_month_balance.item_id and Year(a.b_date) = Year(Getdate()) and month(a.b_date) = 2 and sc_month_balance.service_center = a.service_center),0.0),
        @m3 = isnull((select Sum(a.debit)/(case @all_out when 0 then 1 else @all_out end) from DBA.sc_month_balance as a where a.item_id = sc_month_balance.item_id and Year(a.b_date) = Year(Getdate()) and month(a.b_date) = 3 and sc_month_balance.service_center = a.service_center),0.0),
        @m4 = isnull((select Sum(a.debit)/(case @all_out when 0 then 1 else @all_out end) from DBA.sc_month_balance as a where a.item_id = sc_month_balance.item_id and Year(a.b_date) = Year(Getdate()) and month(a.b_date) = 4 and sc_month_balance.service_center = a.service_center),0.0),
        @m5 = isnull((select Sum(a.debit)/(case @all_out when 0 then 1 else @all_out end) from DBA.sc_month_balance as a where a.item_id = sc_month_balance.item_id and Year(a.b_date) = Year(Getdate()) and month(a.b_date) = 5 and sc_month_balance.service_center = a.service_center),0.0),
        @m6 = isnull((select Sum(a.debit)/(case @all_out when 0 then 1 else @all_out end) from DBA.sc_month_balance as a where a.item_id = sc_month_balance.item_id and Year(a.b_date) = Year(Getdate()) and month(a.b_date) = 6 and sc_month_balance.service_center = a.service_center),0.0),
        @m7 = isnull((select Sum(a.debit)/(case @all_out when 0 then 1 else @all_out end) from DBA.sc_month_balance as a where a.item_id = sc_month_balance.item_id and Year(a.b_date) = Year(Getdate()) and month(a.b_date) = 7 and sc_month_balance.service_center = a.service_center),0.0),
        @m8 = isnull((select Sum(a.debit)/(case @all_out when 0 then 1 else @all_out end) from DBA.sc_month_balance as a where a.item_id = sc_month_balance.item_id and Year(a.b_date) = Year(Getdate()) and month(a.b_date) = 8 and sc_month_balance.service_center = a.service_center),0.0),
        @m9 = isnull((select Sum(a.debit)/(case @all_out when 0 then 1 else @all_out end) from DBA.sc_month_balance as a where a.item_id = sc_month_balance.item_id and Year(a.b_date) = Year(Getdate()) and month(a.b_date) = 9 and sc_month_balance.service_center = a.service_center),0.0),
        @m10 = isnull((select Sum(a.debit)/(case @all_out when 0 then 1 else @all_out end) from DBA.sc_month_balance as a where a.item_id = sc_month_balance.item_id and Year(a.b_date) = Year(Getdate()) and month(a.b_date) = 10 and sc_month_balance.service_center = a.service_center),0.0),
        @m11 = isnull((select Sum(a.debit)/(case @all_out when 0 then 1 else @all_out end) from DBA.sc_month_balance as a where a.item_id = sc_month_balance.item_id and Year(a.b_date) = Year(Getdate()) and month(a.b_date) = 11 and sc_month_balance.service_center = a.service_center),0.0),
        @m12 = isnull((select Sum(a.debit)/(case @all_out when 0 then 1 else @all_out end) from DBA.sc_month_balance as a where a.item_id = sc_month_balance.item_id and Year(a.b_date) = Year(Getdate()) and month(a.b_date) = 12 and sc_month_balance.service_center = a.service_center),0.0),
        @amount1 = isnull((select Sum(a.debit) from DBA.sc_month_balance as a where a.item_id = sc_month_balance.item_id and Year(a.b_date) = Year(Getdate()) and month(a.b_date) = 1 and sc_month_balance.service_center = a.service_center),0.0),
        ...
        @m1_p = isnull((select Sum(a.debit)/(case @all_out when 0 then 1 else @all_out end) from DBA.sc_month_balance as a where a.item_id = sc_month_balance.item_id and Year(a.b_date) = Year(Getdate())-1 and month(a.b_date) = 1 and sc_month_balance.service_center = a.service_center),0.0),
        @m2_p = isnull((select Sum(a.debit)/(case @all_out when 0 then 1 else @all_out end) from DBA.sc_month_balance as a where a.item_id = sc_month_balance.item_id and Year(a.b_date) = Year(Getdate())-1 and month(a.b_date) = 2 and sc_month_balance.service_center = a.service_center),0.0),
        @m3_p = isnull((select Sum(a.debit)/(case @all_out when 0 then 1 else @all_out end) from DBA.sc_month_balance as a where a.item_id = sc_month_balance.item_id and Year(a.b_date) = Year(Getdate())-1 and month(a.b_date) = 3 and sc_month_balance.service_center = a.service_center),0.0),
        @m4_p = isnull((select Sum(a.debit)/(case @all_out when 0 then 1 else @all_out end) from DBA.sc_month_balance as a where a.item_id = sc_month_balance.item_id and Year(a.b_date) = Year(Getdate())-1 and month(a.b_date) = 4 and sc_month_balance.service_center = a.service_center),0.0),
        @m5_p = isnull((select Sum(a.debit)/(case @all_out when 0 then 1 else @all_out end) from DBA.sc_month_balance as a where a.item_id = sc_month_balance.item_id and Year(a.b_date) = Year(Getdate())-1 and month(a.b_date) = 5 and sc_month_balance.service_center = a.service_center),0.0),
        @m6_p = isnull((select Sum(a.debit)/(case @all_out when 0 then 1 else @all_out end) from DBA.sc_month_balance as a where a.item_id = sc_month_balance.item_id and Year(a.b_date) = Year(Getdate())-1 and month(a.b_date) = 6 and sc_month_balance.service_center = a.service_center),0.0),
        @m7_p = isnull((select Sum(a.debit)/(case @all_out when 0 then 1 else @all_out end) from DBA.sc_month_balance as a where a.item_id = sc_month_balance.item_id and Year(a.b_date) = Year(Getdate())-1 and month(a.b_date) = 7 and sc_month_balance.service_center = a.service_center),0.0),
        @m8_p = isnull((select Sum(a.debit)/(case @all_out when 0 then 1 else @all_out end) from DBA.sc_month_balance as a where a.item_id = sc_month_balance.item_id and Year(a.b_date) = Year(Getdate())-1 and month(a.b_date) = 8 and sc_month_balance.service_center = a.service_center),0.0),
        @m9_p = isnull((select Sum(a.debit)/(case @all_out when 0 then 1 else @all_out end) from DBA.sc_month_balance as a where a.item_id = sc_month_balance.item_id and Year(a.b_date) = Year(Getdate())-1 and month(a.b_date) = 9 and sc_month_balance.service_center = a.service_center),0.0),
        @m10_p = isnull((select Sum(a.debit)/(case @all_out when 0 then 1 else @all_out end) from DBA.sc_month_balance as a where a.item_id = sc_month_balance.item_id and Year(a.b_date) = Year(Getdate())-1 and month(a.b_date) = 10 and sc_month_balance.service_center = a.service_center),0.0),
        @m11_p = isnull((select Sum(a.debit)/(case @all_out when 0 then 1 else @all_out end) from DBA.sc_month_balance as a where a.item_id = sc_month_balance.item_id and Year(a.b_date) = Year(Getdate())-1 and month(a.b_date) = 11 and sc_month_balance.service_center = a.service_center),0.0),
        @m12_p = isnull((select Sum(a.debit)/(case @all_out when 0 then 1 else @all_out end) from DBA.sc_month_balance as a where a.item_id = sc_month_balance.item_id and Year(a.b_date) = Year(Getdate())-1 and month(a.b_date) = 12 and sc_month_balance.service_center = a.service_center),0.0),
      ...
      update #t set Jan_sales = @m1,Feb_sales = @m2,Mar_sales = @m3,Apr_sales = @m4,May_sales = @m5,
        Jun_sales = @m6,Jul_sales = @m7,Aug_sales = @m8,Sep_sales = @m9,Oct_sales = @m10,Nov_sales = @m11,Dec_sales = @m12,
        ...
        Jan_sales_p = @m1_p,Feb_sales_p = @m2_p,Mar_sales_p = @m3_p,Apr_sales_p = @m4_p,May_sales_p = @m5_p,Jun_sales_p = @m6_p,
        Jul_sales_p = @m7_p,Aug_sales_p = @m8_p,Sep_sales_p = @m9_p,Oct_sales_p = @m10_p,Nov_sales_p = @m11_p,Dec_sales_p = @m12_p,
