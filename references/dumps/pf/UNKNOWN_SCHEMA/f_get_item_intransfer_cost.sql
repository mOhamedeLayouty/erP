-- PF: UNKNOWN_SCHEMA.f_get_item_intransfer_cost
-- proc_id: 443
-- generated_at: 2025-12-29T13:53:28.820Z

create function DBA.f_get_item_intransfer_cost( in as_item varchar(50),in an_center integer,in an_location integer,in an_store integer ) 
returns numeric(20,7)
//V1.1 Beta
begin
  declare @trans_cost numeric(20,3);
  //
  select Sum(sc_transfer_detail.qty*sc_transfer_detail.price)
    into @trans_cost from sc_transfer_detail
      ,sc_transfer_header
  ...
  //
  /*Select sum( trans_qty * trans_cost) into @trans_cost 
