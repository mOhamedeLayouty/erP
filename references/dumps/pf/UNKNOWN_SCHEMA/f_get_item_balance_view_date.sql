-- PF: UNKNOWN_SCHEMA.f_get_item_balance_view_date
-- proc_id: 383
-- generated_at: 2025-12-29T13:53:28.803Z

create function DBA.f_get_item_balance_view_date( in as_item varchar(50),in an_center integer,in an_location integer,in an_store integer,in ad_date date default today(),in at_time time default now() )  /* @parameter_name parameter_type [= default_value], ... */
...
begin
  ...
  select Sum(sc_items_transaction.trans_qty)
    ...
    and(trans_date < ad_date or(credit_date = ad_date and trans_time < at_time)));
  //Debit
  select sum(sc_items_transaction.trans_qty)
