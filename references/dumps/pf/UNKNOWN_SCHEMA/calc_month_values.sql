-- PF: UNKNOWN_SCHEMA.calc_month_values
-- proc_id: 424
-- generated_at: 2025-12-29T13:53:28.814Z

create procedure DBA.calc_month_values()
//
begin
      ...
      while @smonth <= @emonth loop //Month
        ...
        for centers as curs insensitive cursor for //Centers
          ...
          for locations as locs insensitive cursor for //Locations
            ...
            select DBA.get_kpi_value(@kpi_code,@year,@month,center_id,ar_location) into @kpi_value;
            if not exists(select * from kpi_monthly where year = @year and month = @month
                and service_center = center_id and location_id = ar_location) then
              execute immediate 'insert into kpi_monthly( year,month,'
                 || @kpi_code || ', service_center, location_id) values(' || @year || ',' || @month || ','
                 || @kpi_value || ',' || center_id || ',' || ar_location || ');'
            else
              // execute immediate 'update kpi_monthly set' || @kpi_code || '=' || @kpi_value || 'where year =' || 
              //    @year || 'and month =' || @month || ';'
              execute immediate('update kpi_monthly set '+@kpi_code+' = '+string(@kpi_value)+'where year ='
                +string(@year)+'and month ='+string(@month)+'and service_center ='+string(center_id)+'and location_id ='+string(ar_location))
