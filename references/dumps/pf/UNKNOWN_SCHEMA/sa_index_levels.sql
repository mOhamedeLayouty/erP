-- PF: UNKNOWN_SCHEMA.sa_index_levels
-- proc_id: 78
-- generated_at: 2025-12-29T13:53:28.714Z

create procedure dbo.sa_index_levels( 
...
result( 
  ...
  IndexType char(4),
  Levels integer ) dynamic result sets 1
begin
  ...
  declare first_owner dynamic scroll cursor for
    select rtrim(user_name)
      ...
      and t.creator = u.user_id;
  declare local temporary table IndexLevels(
  ...
  if tbl_name is null then
    ...
    tabs: loop
      ...
      call dbo.sa_internal_index_levels(tname,uname);
      if sqlcode <> 0 then leave tabs end if
    end loop tabs;
    close tables
  else
  ...
  select * from IndexLevels order by TableName asc,IndexName asc
end
