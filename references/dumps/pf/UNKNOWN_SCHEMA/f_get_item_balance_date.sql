-- PF: UNKNOWN_SCHEMA.f_get_item_balance_date
-- proc_id: 382
-- generated_at: 2025-12-29T13:53:28.802Z

create function DBA.f_get_item_balance_date( in as_item varchar(50),in an_center integer,in an_location integer,in an_store integer,in ad_date date default today(),in at_time time default '23:59:59.999999' )  /* @parameter_name parameter_type [= default_value], ... */
returns numeric(20,7)
//V1.1 set time as last day time.
begin
  ...
  select Sum(sc_credit_detail.qty)
    ...
    and(credit_date < ad_date or(credit_date = ad_date and trans_time < at_time)));
  //Debit
  select sum(sc_debit_detail.qty)
    ...
    and(debit_date < ad_date or(debit_date = ad_date and trans_time < at_time)));
  //Return
