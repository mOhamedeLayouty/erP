-- PF: UNKNOWN_SCHEMA.get_budget_month
-- proc_id: 403
-- generated_at: 2025-12-29T13:53:28.808Z

create function ledger.get_budget_month( in arg_year integer,in arg_month integer ) 
...
begin
  ...
  select sum(distinct budg_value_1),
    ...
    where(account.acc_no = budget.acc_no) and(budget.budg_year = arg_year)
